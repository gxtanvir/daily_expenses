...
class _SearchExpenseState extends ConsumerState<SearchExpense>{
  ...
  void initState() {
    super.initState();
    _expenses = ref.read(manageExpenseProvider).where((element) => element.bookId == widget.book.id).toList();
  }

  // Search Expense
  void searchExpense(String query){
    final expenses = ref.watch(manageExpenseProvider).where((element) => element.bookId == widget.book.id).toList();
    final searchedExpenses = expenses.where((expense) => expense.remarks.toLowerCase().contains(query.toLowerCase())).toList();
    setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _expenses.isEmpty ? const Padding(padding: EdgeInsets.symmetric(horizontal: 15, vertical: 10), child: 
      Center(child: Text("Sorry no expense found with this name"),),) : CustomScrollView(
        slivers: [
          SliverToBoxAdapter(
